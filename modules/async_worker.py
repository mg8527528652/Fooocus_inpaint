...
class AsyncTask:
    def __init__(self, args):
        ...
        # args.reverse()
        self.generate_image_grid = args['generate_image_grid']
        ...
        # self.loras = get_enabled_loras([(bool(args['loras'][_][0]), str(args['loras'][_][1]), float(args['loras'][_][2])) for _ in
        #                                 range(default_max_lora_number)])
        self.loras = args['loras']
        self.input_image_checkbox = args['input_image_checkbox']
        ...
        self.inpaint_mask_image_upload = args['inpaint_mask_image_upload']

        self.disable_preview = args['disable_preview']
        self.disable_intermediate_results = args['disable_intermediate_results']
        self.disable_seed_increment = args['disable_seed_increment']
        self.black_out_nsfw = args['black_out_nsfw']
        self.adm_scaler_positive = args['adm_scaler_positive']
        self.adm_scaler_negative = args['adm_scaler_negative']
        self.adm_scaler_end = args['adm_scaler_end']
        self.adaptive_cfg = args['adaptive_cfg']
        self.clip_skip = args['clip_skip']
        self.sampler_name = args['sampler_name']
        self.scheduler_name = args['scheduler_name']
        self.vae_name = args['vae_name']
        self.overwrite_step = args['overwrite_step']
        self.overwrite_switch = args['overwrite_switch']
        self.overwrite_width = args['overwrite_width']
        self.overwrite_height = args['overwrite_height']
        self.overwrite_vary_strength = args['overwrite_vary_strength']
        self.overwrite_upscale_strength = args['overwrite_upscale_strength']
        self.mixing_image_prompt_and_vary_upscale = args['mixing_image_prompt_and_vary_upscale']
        self.mixing_image_prompt_and_inpaint = args['mixing_image_prompt_and_inpaint']
        self.debugging_cn_preprocessor = args['debugging_cn_preprocessor']
        self.skipping_cn_preprocessor = args['skipping_cn_preprocessor']
        self.canny_low_threshold = args['canny_low_threshold']
        self.canny_high_threshold = args['canny_high_threshold']
        self.refiner_swap_method = args['refiner_swap_method']
        self.controlnet_softness = args['controlnet_softness']
        self.freeu_enabled = args['freeu_enabled']
        self.freeu_b1 = args['freeu_b1']
        self.freeu_b2 = args['freeu_b2']
        self.freeu_s1 = args['freeu_s1']
        self.freeu_s2 = args['freeu_s2']
        self.debugging_inpaint_preprocessor = args['debugging_inpaint_preprocessor']
        self.inpaint_disable_initial_latent = args['inpaint_disable_initial_latent']
        self.inpaint_engine = args['inpaint_engine']
        self.inpaint_strength = args['inpaint_strength']
        self.inpaint_respective_field = args['inpaint_respective_field']
        self.inpaint_advanced_masking_checkbox = args['inpaint_advanced_masking_checkbox']
        self.invert_mask_checkbox = args['invert_mask_checkbox']
        self.inpaint_erode_or_dilate = args['inpaint_erode_or_dilate']
        ...
        self.save_metadata_to_images = args['save_metadata_to_images'] if not args_manager.args.disable_metadata else False
        self.metadata_scheme = MetadataScheme(
            args['metadata_scheme']) if not args_manager.args.disable_metadata else MetadataScheme.FOOOCUS

        # self.cn_tasks = {x: [] for x in ip_list}
        # for _ in range(modules.config.default_controlnet_image_count):
        #     cn_img = args['cn_img']
        #     cn_stop = args['cn_stop']
        #     cn_weight = args['cn_weight']
        #     cn_type = args['cn_type']
        #     if cn_img is not None:
        #         self.cn_tasks[cn_type].append([cn_img, cn_stop, cn_weight])
        self.cn_tasks = args['cn_tasks']
        self.debugging_dino = args['debugging_dino']
        self.dino_erode_or_dilate = args['dino_erode_or_dilate']
        self.debugging_enhance_masks_checkbox = args['debugging_enhance_masks_checkbox']
        ...
        self.enhance_ctrls = args['enhance_ctrls']
        # for _ in range(modules.config.default_enhance_tabs):
        #     enhance_enabled = args['enhance_enabled']
        #     enhance_mask_dino_prompt_text = args['enhance_mask_dino_prompt_text']
        #     enhance_prompt = args['enhance_prompt']
        #     enhance_negative_prompt = args['enhance_negative_prompt']
        #     enhance_mask_model = args['enhance_mask_model']
        #     enhance_mask_cloth_category = args['enhance_mask_cloth_category']
        #     enhance_mask_sam_model = args['enhance_mask_sam_model']
        #     enhance_mask_text_threshold = args['enhance_mask_text_threshold']
        #     enhance_mask_box_threshold = args['enhance_mask_box_threshold']
        #     enhance_mask_sam_max_detections = args['enhance_mask_sam_max_detections']
        #     enhance_inpaint_disable_initial_latent = args['enhance_inpaint_disable_initial_latent']
        #     enhance_inpaint_engine = args['enhance_inpaint_engine']
        #     enhance_inpaint_strength = args['enhance_inpaint_strength']
        #     enhance_inpaint_respective_field = args['enhance_inpaint_respective_field']
        #     enhance_inpaint_erode_or_dilate = args['enhance_inpaint_erode_or_dilate']
        #     enhance_mask_invert = args['enhance_mask_invert']
        #     if enhance_enabled:
        ...
        #             enhance_mask_invert
        #         ])
        self.should_enhance = args['should_enhance']
...
def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
    ...
    et = time.time()
    print(f'Processing prompt: Time taken to load models: {et - st} seconds')
    if advance_progress:
        current_progress += 1
...
def process_enhance(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path,
    ...
    # positive and negative conditioning aren't available here anymsore, process prompt again
    ...
    imgs, current_progress = process_task(all_steps, async_task, callback, controlnet_canny_path,
...
def handler(async_task: AsyncTask):
    ...
    print(f'apply_patch_settings time: {time.time() - st:.2f} seconds')
    ...
    print(termcolor.colored(f'apply_image_input time: {time.time() - st:.2f} seconds', 'cyan'))
    # Load or unload CNs
    ...
    print(termcolor.colored(f'process_prompt time: {time.time() - st:.2f} seconds', 'cyan'))
    if len(goals) > 0:
        current_progress += 1
    ...
    if 'inpaint' in goals:
        try:
            ...
            print(termcolor.colored(f'apply_inpaint time: {time.time() - st:.2f} seconds', 'cyan'))
